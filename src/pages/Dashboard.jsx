...
const Dashboard = ({ defaultDomain }) => {
    ...
    .filter((user) => {
      ...
      if (selectedSubdomains.length > 0) {
        ...
        if (
          user.designTasks &&
          user.designTasks[0] &&
          user.designTasks[0].subdomain
        ) {
          userSubs.push(
            ...processSubdomainString(user.designTasks[0].subdomain),
          );
        }
        if (
          user.managementTasks &&
          user.managementTasks[0] &&
          user.managementTasks[0].subdomain
        ) {
          userSubs.push(
            ...processSubdomainString(user.managementTasks[0].subdomain),
          );
        }
      ...
      return (
        matchesSearch && matchesDate && matchesSubdomain && matchesSubmitted
      );
    })
    .sort((a, b) => {
  ...
  const [pageSize, setPageSize] = useState(100); // Default 100 users per page
  ...
  const handleExportCSV = () => {
    ...
    const rows = processedUsers.map((u) => [
      ...
      u.management,
      u.meetingTime
        ? new Date(u.meetingTime).toLocaleString()
        : "Not Scheduled",
    ]);
    ...
    link.setAttribute(
      "download",
      `mfc_recruitment_export_${new Date().toISOString()}.csv`,
    );
    ...
    <div className="container" style={{ maxWidth: "1600px" }}>
      ...
      >
        <div>
          ...
          >
            {defaultDomain
              ? defaultDomain.charAt(0).toUpperCase() + defaultDomain.slice(1)
              : "Overview"}{" "}
            Dashboard
          </h1>
      ...
      >
        ...
        >
          ...
          {showUserList
            ...
            : `View ${processedUsers.length} Participants`}
        </Button>
      </div>
                  ...
                  {paginatedUsers.length > 0 ? (
                    paginatedUsers.map((user) => (
                      ...
                      >
                        ...
                        <td
                          style={{ padding: "20px", color: "var(--text-main)" }}
                        >
                          {user.meetingTime ? (
                            <span
                              style={{
                            ...
                            >
                              <span style={{ fontWeight: "600" }}>
                                {new Date(user.meetingTime).toLocaleTimeString(
                                  [],
