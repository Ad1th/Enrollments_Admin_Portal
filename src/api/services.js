...
export const adminService = {
  ...
  getAllUsers: async (adminId, page = 1, limit = 1000) => {
    ...
    const response = await api.get(`/admin/users/${adminId}?page=${page}&limit=${limit}`);
    return response.data;
  },

  getAllUsersByDomain: async (adminId, domain, page = 1, limit = 1000) => {
    // Backend route: /admin/users/:id?domain=tech
    const response = await api.get(`/admin/users/${adminId}?domain=${domain}&page=${page}&limit=${limit}`);
    return response.data;
  },

  getTechUsers: async (adminId, page = 1, limit = 1000) => {
    const response = await api.get(`/admin/userstech/${adminId}?page=${page}&limit=${limit}`);
    return response.data;
  },

  getDesignUsers: async (adminId, page = 1, limit = 1000) => {
    const response = await api.get(`/admin/usersdesign/${adminId}?page=${page}&limit=${limit}`);
    return response.data;
  },

  getManagementUsers: async (adminId, page = 1, limit = 1000) => {
    const response = await api.get(`/admin/usersmanagement/${adminId}?page=${page}&limit=${limit}`);
    return response.data;
  },
  ...
  updateUserStatus: async (regno, statusUpdates) => {
    ...
    const response = await api.put(`/admin/updatestatus/update`, { regno, ...statusUpdates });
    return response.data;
  }
};
